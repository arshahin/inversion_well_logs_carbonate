...
nruns=3; %%%%% Number of runs(cases) 
niter=300; % number of iteration per run
...
% % % % mksz=70; %%%%% MarkerSize in x-plot
% % % % figure(504)
% % % % scatter3(ai(kks:kkf),vr(kks:kkf),resist(kks:kkf),mksz,phiv(kks:kkf),'filled')
% % % % xlabel('Acoustic Impedance[Vp*Den](Km/sec*gr/cm3)','FontSize',fs)
% % % % ylabel('Velocity Ratio [Vp/Vs]','FontSize',fs)
% % % % zlabel('Resistivity [log10](ohm-m)','FontSize',fs)
% % % % grid on
% % % % cmin=0.0; cmax=max(phiv(kks:kkf));
% % % % c = colorbar; c.Label.String = 'Vuggy Porosity (v/v)';
% % % % colormap(flipud(jet)); caxis([cmin cmax])
...
phiv_glob=zeros(np,nruns); 
phii_glob=zeros(np,nruns); 
phit_glob=zeros(np,nruns); 
swv_glob=zeros(np,nruns); 
swi_glob=zeros(np,nruns);
swt_glob=zeros(np,nruns);
lamdai_glob=zeros(1,nruns); %%%% matrix lithology exponent 
lamdav_glob=zeros(1,nruns); %%%% vuggy lithology exponent
Lik_glob=zeros(1,nruns); %%%% L parameter for intergranular porosity associated with bulk modulus
Limu_glob=zeros(1,nruns); %%%% L parameter for intergranular porosity associated with shear modulus
Cx_glob=zeros(1,nruns); %%%% salinity of flushed zone
phic_glob=zeros(1,nruns); %%%% critical porosity 
ks_glob=zeros(1,nruns); %%%% matrix bulk modulus 
mus_glob=zeros(1,nruns); %%%% matrix shear modulus  
rhos_glob=zeros(1,nruns); %%%% matrix density
...
% % % % % %         %%%%% plotting of SEG 2019
        ...
        while(jtemp<=niter-1)
            ...
            lamdai_current(jtemp)=lamdai_mod;
            Cx_current(jtemp)=Cx_mod;
            phic_current(jtemp)=phic_mod;
            rhos_current(jtemp)=rhos_mod;
            ks_current(jtemp)=ks_mod;
            mus_current(jtemp)=mus_mod;
            Lik_current(jtemp)=Lik_mod;
            Limu_current(jtemp)=Limu_mod;
            
            
            for jp=1:np
                phii_current(jp,jtemp)=phii_mod(jp);
                phit_current(jp,jtemp)=phit_mod(jp);
                phiv_current(jp,jtemp)=phit_mod(jp)-phii_mod(jp);
                swt_current(jp,jtemp)=swt_mod(jp);
                swi_current(jp,jtemp)=swt_mod(jp);
                swv_current(jp,jtemp)=swt_mod(jp);
            end
...
% % %                 gphivw(jrun,jtemp,:)=phiv_mod.*swv_mod;   
        ...
        %%%%%%%%%% grab all statistics
        lamdai_glob(jrun)=lamdai_mod; 
        Cx_glob(jrun)=Cx_mod;
        phic_glob(jrun)=phic_mod;
        rhos_glob(jrun)=rhos_mod;
        ks_glob(jrun)=ks_mod;
        mus_glob(jrun)=mus_mod;
        Lik_glob(jrun)=Lik_mod;
        Limu_glob(jrun)=Limu_mod;
        phit_glob(:,jrun)=phit_mod;
        phii_glob(:,jrun)=phii_mod;
        phiv_glob(:,jrun)=phiv_mod;
        swt_glob(:,jrun)=swt_mod;
        swi_glob(:,jrun)=swt_mod;
        swv_glob(:,jrun)=swt_mod;
        phiiw_glob(:,jrun)=phii_mod.*swi_mod;
        phivw_glob(:,jrun)=phiv_mod.*swv_mod;
        sot_mod=1-swt_mod;
        sot_glob(:,jrun)=sot_mod;
        sgt_glob(:,jrun)=1-swt_mod-sot_mod;
end
...
if nruns>1
    for jp=1:np
        phii_mean(jp,1)=mean(phii_glob(jp,:));
        phit_mean(jp,1)=mean(phit_glob(jp,:));
        phiv_mean(jp,1)=phit_mean(jp,1)-phii_mean(jp,1);
        swt_mean(jp,1)=mean(swt_glob(jp,:));
        swi_mean(jp,1)=swt_mean(jp,1);
        swv_mean(jp,1)=swt_mean(jp,1);
    end
    phiiw_mean=phii_mean.*swi_mean;
    phivw_mean=phiv_mean.*swv_mean;
    sot_mean=1-swt_mean;
    sgt_mean=1-swt_mean-sot_mean;
    lamdai_mean=mean(lamdai_glob); %%%% matrix lithology exponent 
    Cx_mean=mean(Cx_glob); %%%% salinity of flushed zone
    phic_mean=mean(phic_glob);
    rhos_mean=mean(rhos_glob);
    ks_mean=mean(ks_glob);
    mus_mean=mean(mus_glob);
    Lik_mean=mean(Lik_glob);
    Limu_mean=mean(Limu_glob);
else
    err_plt_min=min(err_plt);
    inerr1=find(err_plt==err_plt_min);
    ...
    lamdai_mean=lamdai_current(inerr-1);
    Cx_mean=Cx_current(inerr-1);
    phic_mean=phic_current(inerr-1);
    Lik_mean=Lik_current(inerr-1);
    Limu_mean=Limu_current(inerr-1);
    ks_mean=ks_current(inerr-1);
    mus_mean=mus_current(inerr-1);
    rhos_mean=rhos_current(inerr-1);  
    phii_mean=phii_current(:,inerr-1);
    phit_mean=phit_current(:,inerr-1);
    phiv_mean=phit_mean-phii_mean;
    swt_mean=swt_current(:,inerr-1);
    swi_mean=swt_mean;
    swv_mean=swt_mean;
    phiiw_mean=phii_mean.*swi_mean;
    phivw_mean=phiv_mean.*swv_mean;
...
% %     phii_mean=phii_current(:,jtemp-1);
% %     phit_mean=phit_current(:,jtemp-1);
% %     phiv_mean=phit_mean-phii_mean;
% %     swt_mean=swt_current(:,jtemp-1);
% %     swi_mean=swt_mean;
% %     swv_mean=swt_mean;
% %     phiiw_mean=phii_mean.*swi_mean;
% %     phivw_mean=phiv_mean.*swv_mean;
% %     sot_mean=1-swt_mean;
% %     sgt_mean=1-swt_mean-sot_mean;
% %     lamdai_mean=lamdai_current(jtemp-1); %%%% matrix lithology exponent 
% %     Cx_mean=Cx_current(jtemp-1); %%%% salinity of flushed zone
% %     phic_mean=phic_current(jtemp-1);
% %     rhos_mean=rhos_current(jtemp-1);
% %     ks_mean=ks_current(jtemp-1);
% %     mus_mean=mus_current(jtemp-1);
% %     Lik_mean=Lik_current(jtemp-1);
% %     Limu_mean=Limu_current(jtemp-1);
   

end

figure(101); subplot(2,1,1); plot(err_plt); hold on; subplot(2,1,2); plot(lamdai_current)
    ...
    for jrun=1:nruns
        [dtp_sim_ppd(:,jrun),dts_sim_ppd(:,jrun),vp_sim_ppd(:,jrun),vs_sim_ppd(:,jrun),ai_sim_ppd(:,jrun),si_sim_ppd(:,jrun)...
            ,vr_sim_ppd(:,jrun),pr_sim_ppd(:,jrun),ksat_sim_ppd(:,jrun),musat_sim_ppd(:,jrun),den_sim_ppd(:,jrun),logk,logmu]=...
             sdem_sonic_carbonate_v01...
             (phit_glob(:,jrun),phii_glob(:,jrun),phiv_glob(:,jrun),phic_glob(jrun),Lik_glob(jrun),Limu_glob(jrun),Lvk,Lvmu,...
             swt_glob(:,jrun),sot_glob(:,jrun),sgt_glob(:,jrun),satype,kw,ko,kg,rhow,rhoo,rhog,ks_glob(jrun),mus_glob(jrun),rhos_glob(jrun));


    %%%%%%%%%%%%%% Resistivity
        [resist_sim_ppd(:,jrun),ffac_sim_ppd(:,jrun),rw_sim_ppd(:,jrun),logr]...
            =sdem_resist_carbonate(phit_glob(:,jrun),phii_glob(:,jrun),phiv_glob(:,jrun),swi_glob(:,jrun),swv_glob(:,jrun),lamdai_glob(jrun),lamdav,tmpc,Cx_glob(jrun),wetcase);
...
% % % %     figure(1)
% % % %     subplot(3,3,1)  
% % % %     plot(err_plt(:,runy),'c','LineWidth',lw)
% % % %     xlim([0 jtemp-1]); grid;
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Objective function','FontSize',fs);    title('Objective function','FontSize',fs);
% % % %     set(gca,'FontSize',fs)
% % % %     subplot(3,3,2)
% % % %     plot(lamdai_plt(:,runy),'c','LineWidth',lw); 
% % % %     hold on
% % % %     plot(lamdai_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     plot(lamdai_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Intergranular Lithology Exponent','FontSize',fs);
% % % %     ylim([lamdai_min lamdai_max]);grid; set(gca,'FontSize',fs) 
% % % %     subplot(3,3,3)
% % % %     plot(phic_plt(:,runy),'c','LineWidth',lw); 
% % % %     hold on
% % % %     plot(phic_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     plot(phic_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Critical Porosity(v/v)','FontSize',fs);
% % % %     ylim([phic_min phic_max]); grid; set(gca,'FontSize',fs) 
% % % %     subplot(3,3,4)
% % % %     plot(Cx_plt(:,runy),'c','LineWidth',lw); 
% % % %     hold on
% % % %     plot(Cx_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     plot(Cx_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Salinity (Kppm)','FontSize',fs);
% % % %     ylim([Cx_min Cx_max]); grid; set(gca,'FontSize',fs) 
% % % %     subplot(3,3,5)
% % % %     plot(Lik_plt(:,runy),'c','LineWidth',lw);
% % % %     hold on
% % % %     plot(Lik_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     plot(Lik_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Intergranular L Prameter for Bulk Modulus(Lik) ','FontSize',fs);
% % % %     ylim([Lik_min Lik_max]);grid; set(gca,'FontSize',fs) 
% % % %     subplot(3,3,6)
% % % %     plot(Limu_plt(:,runy),'c','LineWidth',lw);
% % % %     hold on
% % % %     plot(Limu_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     plot(Limu_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Intergranular L Prameter for Shear Modulus(Lik) ','FontSize',fs);
% % % %     ylim([Limu_min Limu_max]); grid;  set(gca,'FontSize',fs) 
% % % %     subplot(3,3,7)
% % % %     plot(rhos_plt(:,runy),'c','LineWidth',lw);
% % % %     hold on
% % % %     plot(rhos_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     plot(rhos_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Matrix Density(gr/cm3) ','FontSize',fs);
% % % %     ylim([rhos_min rhos_max]); grid;  set(gca,'FontSize',fs) 
% % % %     subplot(3,3,8)
% % % %     plot(ks_plt(:,runy),'c','LineWidth',lw);
% % % %     hold on
% % % %     plot(ks_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     plot(ks_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Matrix Bulk Modulus(GPa) ','FontSize',fs);
% % % %     ylim([ks_min ks_max]); grid;  set(gca,'FontSize',fs) 
% % % %     subplot(3,3,9)
% % % %     plot(mus_plt(:,runy),'c','LineWidth',lw);
% % % %     hold on
% % % %     plot(mus_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     plot(mus_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Matrix Shear Modulus(GPa) ','FontSize',fs);
% % % %     ylim([mus_min mus_max]); grid;  set(gca,'FontSize',fs) 
...
% % % %     semilogx(lamdai_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     semilogx(lamdai_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Intergranular Lithology Exponent','FontSize',fs);
% % % %     ylim([lamdai_min lamdai_max]);grid; set(gca,'FontSize',fs) 
% % % %     subplot(3,3,3)
...
% % % %     semilogx(phic_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     semilogx(phic_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Critical Porosity(v/v)','FontSize',fs);
% % % %     ylim([phic_min phic_max]); grid; set(gca,'FontSize',fs) 
% % % %     subplot(3,3,4)
...
% % % %     semilogx(Cx_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     semilogx(Cx_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Salinity (Kppm)','FontSize',fs);
% % % %     ylim([Cx_min Cx_max]); grid; set(gca,'FontSize',fs) 
% % % %     subplot(3,3,5)
...
% % % %     semilogx(Lik_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     semilogx(Lik_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Intergranular L Prameter for Bulk Modulus(Lik) ','FontSize',fs);
% % % %     ylim([Lik_min Lik_max]);grid; set(gca,'FontSize',fs) 
% % % %     subplot(3,3,6)
...
% % % %     semilogx(Limu_true,'b','LineWidth',lw)
% % % %      hold on
% % % %     semilogx(Limu_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Intergranular L Prameter for Shear Modulus(Lik) ','FontSize',fs);
% % % %     ylim([Limu_min Limu_max]); grid;  set(gca,'FontSize',fs) 
% % % %     subplot(3,3,7)
...
% % % %     semilogx(rhos_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     semilogx(rhos_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Matrix Density(gr/cm3) ','FontSize',fs);
% % % %     ylim([rhos_min rhos_max]); grid;  set(gca,'FontSize',fs) 
% % % %     subplot(3,3,8)
...
% % % %     semilogx(ks_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     semilogx(ks_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Matrix Bulk Modulus(GPa) ','FontSize',fs);
% % % %     ylim([ks_min ks_max]); grid;  set(gca,'FontSize',fs) 
% % % %     subplot(3,3,9)
...
% % % %     semilogx(mus_true,'b','LineWidth',lw)
% % % %     hold on
% % % %     semilogx(mus_mean_line,'r','LineWidth',lw)
% % % %     xlim([0 jtemp-1]);
% % % %     xlabel('Iteration number','FontSize',fs);    ylabel('Model parameters','FontSize',fs);    title('Matrix Shear Modulus(GPa) ','FontSize',fs);
% % % %     ylim([mus_min mus_max]); grid;  set(gca,'FontSize',fs) 
...
if nruns>1
        ...
        figure(5)
        subplot(1,4,1)
        plot(phii_glob(:,runy),DEPTH,'c','LineWidth',lw)
        hold on
        plot(phii_mean,DEPTH,'r',phii,DEPTH,'b','LineWidth',lw)
        title('Intergranular porosity(v/v)','FontSize',fst)
        ylabel('Relative Depth(ft)','FontSize',fsy)
        xlim(phi_xlim); ylim(depth_lim_all);grid MINOR; grid on; set(gca,'YDir','reverse','FontSize',fset)       
        subplot(1,4,2)
        plot(phiv_glob(:,runy),DEPTH,'c','LineWidth',lw)
        hold on
        plot(phiv_mean,DEPTH,'r',phiv,DEPTH,'b','LineWidth',lw)
        title('Vuggy porosity(v/v)','FontSize',fst)
        xlim(phi_xlim);ylim(depth_lim_all);grid MINOR ;grid on;set(gca,'YDir','reverse','FontSize',fset)       
        subplot(1,4,3)
        plot(phiiw_glob(:,runy)+phivw_glob(:,runy),DEPTH,'c',phit_glob(:,runy),DEPTH,'y','LineWidth',lw)
        hold on
        ...
        xlim(phi_xlim);ylim(depth_lim_all);grid MINOR;grid on;set(gca,'YDir','reverse','FontSize',fset)       
        subplot(1,4,4)
        plot(swt_glob(:,runy),DEPTH,'c','LineWidth',lw)
        hold on
        plot(swt_mean,DEPTH,'r',swt,DEPTH,'b','LineWidth',lw)
...
if nruns>1
        ...
        dz=1; %%% one feet 
        dd=(1:1:np).*dz;
        aphiimax=max((max(phii_glob))); 
        aphiimin=min(min(phii_glob));  
        aphiilog=linspace(aphiimin,aphiimax,nd);

        bphivmax=max((max(phiv_glob))); 
        bphivmin=min(min(phiv_glob));
        bphivlog=linspace(bphivmin,bphivmax,nd);

        phitw_glob=phiiw_glob+phivw_glob;
        cphitwmax=max((max(phitw_glob))); 
        cphiimin=min(min(phitw_glob));  
        cphitwlog=linspace(cphiimin,cphitwmax,nd);

        dswtmax=max((max(swt_glob))); 
        dswtmin=min(min(swt_glob));  
        dswtlog=linspace(dswtmin,dswtmax,nd);

        for jp=1:np
            phii_ppd(jp,:)=hist(phii_glob(jp,:),aphiilog)./nruns;
            phiv_ppd(jp,:)=hist(phiv_glob(jp,:),bphivlog)./nruns;
            phitw_ppd(jp,:)=hist(phitw_glob(jp,:),cphitwlog)./nruns;
            swt_ppd(jp,:)=hist(swt_glob(jp,:),dswtlog)./nruns;
        end
